...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        long avgTasks1 = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                //.filter(t1 -> inProgressTasks.contains((t1)))
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated().until(LocalDate.now(), ChronoUnit.DAYS))
                //.map(t -> DAYS.between(t.getCreated(),LocalDate.now())
                .reduce(0L, (a, b) -> a = a + b);

         long avgTasks2 = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated())
                .count();
/*
          double avgTasks3 = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated().until(LocalDate.now(), ChronoUnit.DAYS))
                //.map(t -> DAYS.between(t.getCreated(),LocalDate.now())
...
*/
        //Then
        Assert.assertEquals(10, avgTasks1/avgTasks2);
    }
}
